...
static const char* AppInfo_OutDir(void) { return "ERROR"; }
...
int FS_DirectoryIterator(const char* path, int(__cdecl* FS_DirectoryHandlerCallback)(const char* path))
{
	...
	do
	{
		...
		{
			char tpath[MAX_PATH];
			sprintf_s(tpath, "%s/%s", path, file_data.cFileName);
...
int FS_CopyDirectory(char* srcPath, char* destPath, bool overwriteFiles)
{
	...
	HANDLE h = FindFirstFileA(tmp, &file_data);
	if (!h)
	{
		return 1;
	}
